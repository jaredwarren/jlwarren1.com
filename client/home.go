...
func (c *Client) DownloadFaviconIco(ctx context.Context, dest string) (int64, error) {
	...
	u := url.URL{Host: c.Host, Scheme: scheme, Path: "/favicon.ico"}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return 0, err
	}
...
// SetTypeHomePath computes a request path to the setType action of home.
func SetTypeHomePath() string {
	return fmt.Sprintf("/settype")
}

// Set Printer Type
func (c *Client) SetTypeHome(ctx context.Context, path string) (*http.Response, error) {
	req, err := c.NewSetTypeHomeRequest(ctx, path)
	if err != nil {
		return nil, err
	}
	return c.Client.Do(ctx, req)
...
// NewSetTypeHomeRequest create the request corresponding to the setType action endpoint of the home resource.
func (c *Client) NewSetTypeHomeRequest(ctx context.Context, path string) (*http.Request, error) {
	scheme := c.Scheme
	if scheme == "" {
		scheme = "https"
	}
