...
type (
	...
	// ResumeHomeCommand is the command line data structure for the resume action of home
	ResumeHomeCommand struct {
		PrettyPrint bool
	}

	// UpdateResumeHomeCommand is the command line data structure for the updateResume action of home
	UpdateResumeHomeCommand struct {
		PrettyPrint bool
	}
...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "resume",
		Short: `Single Page Resume`,
	}
	tmp3 := new(ResumeHomeCommand)
	sub = &cobra.Command{
		Use:   `home ["/resume"]`,
		Short: ``,
...
func (cmd *DownloadCommand) Run(c *client.Client, args []string) error {
	...
	if rpath == "/.well-known/keybase.txt" {
		fnf = c.DownloadKeybaseTxt
		if outfile == "" {
			outfile = "keybase.txt"
		}
		goto found
...
// Run makes the HTTP request corresponding to the ResumeHomeCommand command.
func (cmd *ResumeHomeCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ResumeHome(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *ResumeHomeCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
}

// Run makes the HTTP request corresponding to the UpdateResumeHomeCommand command.
func (cmd *UpdateResumeHomeCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.UpdateResumeHome(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *UpdateResumeHomeCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
}
